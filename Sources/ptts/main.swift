...
///
/// Usage:
///   ptts "Hello world"                    # Enqueue speech in Loqui broker
...
func printUsage() {
    ...
    NOTE:
        Requires Loqui.app to be running.
        Default mode enqueues speech in Loqui's local broker queue for centralized playback.
    """
...
func sendBrokerCommand(host: String, port: Int, request: BrokerRequest, timeout: TimeInterval = 3.0) async throws -> BrokerResponse {
    ...
    return try await withCheckedThrowingContinuation { continuation in
        ...
        let timeoutWork = DispatchWorkItem {
            ...
            resumed = true
            connection.cancel()
            continuation.resume(throwing: TTSError.serverError("Loqui broker timeout"))
        }
...
func checkBrokerHealth(host: String, brokerPort: Int) async throws {
    ...
    if response.ok != true {
        throw TTSError.serverError(response.error ?? "Loqui broker not healthy")
    }
}
...
func main() async {
    ...
    if cli.stopSpeech {
        do {
            ...
            // Compatibility fallback for older Loqui instances without broker
            do {
