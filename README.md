...
Loqui wraps these into a macOS menubar app with a settings UI, bundles the model weights, and exposes a simple HTTP API that any application can use. Since we are not using the voice cloning model we don't need the user to sign up for the huggingface access. 
...
The `ptts` command lets any application use TTS:
...
The bundled Pi extension intercepts `<voice>` tags from the assistant and speaks them:
...
Loqui runs as a menubar app. It starts a local HTTP server on port 18080. Applications send text via HTTP or the `ptts` CLI, and Loqui streams audio to ffplay (or returns raw PCM/WAV for custom handling).
